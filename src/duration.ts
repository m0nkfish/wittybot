...
export class Duration {
  ...
  combine = (f: (a: number, b: number) => number) => (other: DurationLike): Duration => {
    if (other instanceof Duration) {
      return new Duration(f(this.milliseconds, other.milliseconds))
    } else {
      return this.combine(f)(new Duration(other))
    }
  }

  subtract = this.combine((a, b) => a - b)
  add = this.combine((a, b) => a + b)
